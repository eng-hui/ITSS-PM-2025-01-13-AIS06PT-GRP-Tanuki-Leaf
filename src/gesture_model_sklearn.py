...
# Find indices for which the one-hot encoding is all zeros.
zero_row_indices = np.where(np.all(y_encoded == 0, axis=1))[0]
print("Rows with all zero values in y_encoded:", zero_row_indices)
...
# Split data (use y_labels for RF)   
...
print("Accuracy:", accuracy_score(y_test_rf, y_pred_r))
print("\nRandom Forest Classification Report:\n", classification_report(y_test_rf, y_pred_r))
joblib.dump(model, 'hand_gesture_rf.joblib')  # Save to disk

# Split data for Decision Tree (80% train, 20% test example)=================================================================
...
print("\nDecision Tree Classification Report:\n", classification_report(y_test_dt, y_pred_dt, target_names=label_encoder.classes_))
joblib.dump(model, 'hand_gesture_dt.joblib')  # Save to disk

# Use the previously computed y_labels from LabelEncoder (defined above)
# Split data for SVM and KNN (using same split for both models)
...
joblib.dump(model, 'hand_gesture_svm.joblib')  # Save to disk

# K-Nearest Neighbors Classifier
# Using 5 neighbors as an example.
...
print("\nKNN Classification Report:\n", classification_report(y_test_svm, y_pred_knn, target_names=label_encoder.classes_))

joblib.dump(model, 'hand_gesture_knn.joblib')  # Save to disk